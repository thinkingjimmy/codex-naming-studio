...
6. Tell the user briefly that the GUI has been updated, then immediately re-arm `scripts/watch-naming-request.mjs --project-dir /absolute/path/to/user/workspace --timeout 1800` for the next GUI click. If the watcher later times out, tell the user the workbench is still open and whether `latestPendingRequest` exists.
